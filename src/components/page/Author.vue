<template>
  <v-container fluid grid-list-md text-xs-center>
    <v-layout row wrap>
      <v-flex xs12 sm3>
        <referrer-host
          :author_id="this.$route.params.id"
          :referrerHost="getReferrerHost"
        />
      </v-flex>
      <v-flex xs12 sm3>
        <metrics :chartData="getMetricsData"/>
      </v-flex>
      <v-flex xs12 sm6>
        <axis-history :chartData="getDetailsData"/>
      </v-flex>
    </v-layout>
    <v-layout row wrap>
      <v-flex xs12>
      </v-flex>
    </v-layout>
  </v-container>
</template>

<script>
  import { mapGetters } from 'vuex'
  import Enclosures from '../widget/Enclosures'

  import ReferrerHost from '../widget/ReferrerHost'
  import Metrics from '../widget/element/Metrics'
  import EnclosuresNew from '../widget/EnclosuresNew'
  import AxisHistory from '../widget/element/AxisHistory'

  export default {
    name: 'Enclosures',
    data () {
      return {
        enc: 0,
        dataJson: {},
        chartOptions: {
          legend: {
            display: true
          },
          responsive: true,
          maintainAspectRatio: false
        },
        chartData: {
          labels: ['January', 'February', 'March', 'April', 'May', 'June', 'July'],
          datasets: [
            {
              label: 'Data One',
              backgroundColor: '#f87979',
              data: [40, 39, 10, 40, 39, 80, 40]
            }
          ]
        }
      }
    },
    computed: {
      // Mix your getter(s) into computed with the object spread operator
      ...mapGetters([
        'getData',
        'getDataField',
        'getEnclosures',
        'getMetrics',
        'getDetails'
      ]),
      // Create another computed property to call your mapped getter while passing the argument
      getMetricsData () {
        for (let prop in this.getDetails) {
          if (this.getDetails.hasOwnProperty(prop)) {
            console.log(this.getDetails[prop]['key'], this.$route.params.id)
            if (this.getDetails[prop]['key'] === parseInt(this.$route.params.id)) {
              return this.getDetails[prop]['metrics']
            }
          }
        }
        // return this.getDetails
      },
      getEnclosuresData () {
        return this.getEnclosures
      },
      getReferrerHost () {
        for (let prop in this.getDetails) {
          if (this.getDetails.hasOwnProperty(prop)) {
            if (this.getDetails[prop]['key'] === parseInt(this.$route.params.id)) {
              return this.getDetails[prop]['fields']['referrer.host']
            }
          }
        }
      },
      getDetailsData () {
        for (let prop in this.getDetails) {
          if (this.getDetails.hasOwnProperty(prop)) {
            if (this.getDetails[prop]['key'] === parseInt(this.$route.params.id)) {
              return this.getDetails[prop]['details']
            }
          }
        }
      },
      getDetailsDataDetails () {
        console.log(this.$route.params.details_id)
        for (let prop in this.getDetails) {
          if (this.getDetails.hasOwnProperty(prop)) {
            if (this.getDetails[prop]['key'] === parseInt(this.$route.params.id)) {
              return this.getDetails[prop]['details']
            }
          }
        }
      }
    },
    methods: {},
    mounted () {
      // this.$store.dispatch('getData')
    },
    components:
      {
        AxisHistory,
        EnclosuresNew,
        Metrics,
        ReferrerHost,
        Enclosures
      }
  }
</script>

<style scoped>

</style>
